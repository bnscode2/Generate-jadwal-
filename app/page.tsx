...
export default function AdministrativeDashboard() {
  ...
  const handleExportExcel = async () => {
    try {
      ...
      const schoolName = (schoolProfile?.nama_sekolah || currentUser?.nama_sekolah || 'SMP Negeri 1 AI').toUpperCase();
      const schoolYear = (schoolProfile?.tahun_ajaran || 'TAHUN AJARAN 2026/2027').toUpperCase();
      ...
      a.href = url;
      a.download = `jadwal_pelajaran_${schoolProfile?.nama_sekolah?.toLowerCase().replace(/\s+/g, '_') || 'sekolah'}.xlsx`;
      document.body.appendChild(a);
      a.click();
      document.body.removeChild(a);
      window.URL.revokeObjectURL(url);

      setLogMessages(prev => [`Jadwal pelajaran resmi berhasil diekspor ke Excel (.xlsx) dengan visual lengkap & daftar kode referensi.`, ...prev]);
    } catch (err: any) {
